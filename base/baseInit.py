...
def init(devices):
    ...
    os.popen("adb -s %s install -r -s %s" % (devices, PATH("../app/appium-uiautomator2-server-v0.1.9.apk")))
    os.popen("adb -s %s install -r -s %s" % (devices, PATH("../app/appium-uiautomator2-server-debug-androidTest.apk")))
